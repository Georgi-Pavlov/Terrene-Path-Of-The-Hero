...
- **A growing hero roster**, each with a full, unique skill kit - Veyrik, Lone Druid (with his Spirit Bear), Abaddon, Kunkka, Ancient Apparition, Crystal Maiden, Naga Siren, Mirana, and Luna so far.
...
1. Each zone on the map with a green dot offers one or more heroes - browse them with the arrows, pick a skill, and press **Accept**. Your hero and that skill are locked in for the rest of the run; starting over requires a fresh "New Game."
2. Fight enemies in turn-based battles. Each turn you may use only one of: movement, attack, skill, or item. Enemies will chase or flee, and you can only attack what's in range - ranged heroes and skills require choosing a target as well.
3. Killing enemies earns XP (for leveling up) and gold (to spend at the Shop). Potions and stat-boost items can be used any time, free of your turn's move/attack allowance.
4. Clear every stage in a zone and you may be challenged to a duel by one of that zone's rival heroes - win it and you unlock the next zone, plus take their bounty in XP and gold. Every other hero on the map is doing the same thing in the background, and once a hero clears their own zone they'll go hunt for a new one to invade. Check the map after a battle for news of anyone who's fallen, and use the **World Status** screen (next to the Shop) to see the state of the entire world at a glance.
5. If your HP reaches zero, your run ends and your best XP is saved as a high score before you're returned to the main menu.
...
│   ├── Login.tscn          # Existing-player sign in
│   ├── Register.tscn       # New-player account creation
│   ├── PostLogin.tscn      # Continue / New Game landing screen
│   ├── LoadingScreen.tscn  # Reusable "loading bar" transition between scenes
│   ├── Map.tscn            # World map, zone navigation, World Status/Shop entry points
│   ├── Zone.tscn           # Zone detail: hero browsing/selection before a run
│   ├── Battle.tscn         # Turn-based battle UI
│   ├── Shop.tscn           # Item shop
│   ├── WorldStatus.tscn    # Full zone/hero status overview
│   ├── SettingsPopup.tscn  # Music/SFX settings popup
│   └── HowToPlay.tscn      # In-game rules/help screen
├── scripts/
...
│   ├── EnemySkillAI.gd     # Shared skill-scoring brain used by both real fights and the background simulation
│   ├── EnemySkillRange.gd  # Range checks for enemy skills that require a target in reach
│   ├── battle.gd           # Player battle flow: movement, attacks, skills, items
│   ├── Map.gd
│   ├── zone.gd
│   ├── Shop.gd
│   ├── WorldStatus.gd
│   ├── MainMenu.gd
│   ├── Login.gd / Register.gd / PostLogin.gd
│   ├── SettingsManager.gd / SettingsPopup.gd   # Persistent audio settings + their popup
│   ├── AudioManager.gd     # Music/SFX playback (autoload)
│   ├── HowToPlay.gd
│   └── LoadingScreen.gd
└── assets/
    ├── heroes/             # Player-hero portraits/sprites
    ├── heroes skills/      # Skill icons
    ├── enemies/            # Rival-hero and creep portraits/sprites
    ├── zones/              # Zone backgrounds
    ├── battle_areas/       # Battle background art
    ├── items/              # Potion/equipment icons
    └── music/              # Music tracks
```
...
MIT - update this section if you're using a different license.
